...
import { css, T, formatDate, uid, checkBadges } from './theme';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', animation: 'fadeIn 0.2s' }}
      ...
      <div style={{ background: '#111', borderRadius: '20px 20px 0 0', padding: 20, animation: 'slideUp 0.3s', border: '1px solid #1e1e1e', borderBottom: 'none', maxHeight: '90dvh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 0, marginBottom: 16, borderRadius: 8, overflow: 'hidden', border: '1px solid #2a2a2a' }}>
          {[
            ...
            <button key={opt.id} onClick={() => setType(opt.id)} style={{
              ...
              background: type === opt.id ? (opt.id === 'lent' ? T.neonDim : '#2a0010') : '#1a1a1a',
              color: type === opt.id ? (opt.id === 'lent' ? T.neon : T.red) : T.muted,
        ...
        <div style={{ marginBottom: 12 }}>
          <input type="number" style={{ ...css.input, fontSize: 22, ...css.orbitron, color: type === 'lent' ? T.neon : T.red, textAlign: 'center' }}
    ...
    <div style={{ ...css.darkCard, marginTop: 16 }}>
      ...
      {per > 0 && (
        <div style={{ textAlign: 'center', marginBottom: 12 }}>
          ...
          <div style={{ ...css.orbitron, fontSize: 32, fontWeight: 900, color: T.neon }}>₹{per.toLocaleString()}</div>
        </div>
      )}
  ...
  const BorrowCard = ({ b }) => {
    ...
    return (
      <div style={{
        ...
        border: `1px solid ${overdue ? T.red : '#1e1e1e'}`,
        boxShadow: overdue ? `0 0 12px ${T.red}33` : 'none',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <div>
            ...
            {b.dueDate && (
              <div style={{ fontSize: 11, color: overdue ? T.red : T.muted, marginTop: 2 }}>
                {overdue ? '⚠️ OVERDUE' : '📅'} {formatDate(b.dueDate)}
              </div>
            )}
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ ...css.orbitron, fontSize: 18, fontWeight: 900, color: b.type === 'lent' ? T.neon : T.red }}>
              ₹{Number(b.amount).toLocaleString()}
            </div>
            <div style={{
              fontSize: 10, ...css.orbitron, letterSpacing: 1, marginTop: 2,
              color: b.type === 'lent' ? T.neon : T.red,
              background: b.type === 'lent' ? T.neonDim : '#2a0010',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => handleSettle(b.id)} style={{
            ...
            background: T.neonDim, border: `1px solid ${T.neon}`, borderRadius: 8,
            color: T.neon, fontFamily: "'Orbitron', monospace", fontWeight: 700, letterSpacing: 1,
          }}>✓ SETTLED</button>
